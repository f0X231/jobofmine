...
class EmployeeController extends Controller
{
    public function index()
    {
        // Get Banner
        $id = 0;
        $banner = parent::getBanner('employee-program', $id);
        ...
        $data = array();
        $employees = employee::where('is_active', 'Y')->orderBy('order_no', 'asc')->get();
        foreach($employees as $key => $employee) {
            $title      = unserialize($employee->title);
            $slugTH     = $this->make_slug($title['th']);
            $slugEN     = $this->make_slug($title['en']);

            $data[$key]['id']           = $employee->id;
            $data[$key]['title']        = $title;
            $data[$key]['thumbnail']    = unserialize($employee->thumbnail);
            $data[$key]['slug']         = array(
                                                'th'    => 'employee-program/'.$employee->id.'/'.$slugTH,
                                                'en'    => 'employee-program/'.$employee->id.'/'.$slugEN,
                                            );
        }

        $seo = parent::getSEO(  'บริการด้านสุขภาพจิตองค์กร', 
                                ...
                                'คลินิก JOY OF MINDS, แพทย์ที่เชี่ยวชาญด้านจิตเวช, services, บริการด้านสุขภาพจิตองค์กร', 
                                config('global.sitename').$banner[0]['image']['th'],
                                config('global.sitename').'employee-program'    );

        return view('pages.employee', [  'seo'       => $seo,
                                        'banner'    => $banner,
                                        'data'      => $data,
                                        'services'  => $services,
                                        'doctor'    => $doctor       ]);
    ...
    public function detail($id, $name)
    {
        ...
        $seo = parent::getSEO(  $data[0]['title']['th'], 
                                ...
                                config('global.sitename').$banner[0]['image']['th'],
                                config('global.sitename').'employee-program/'.$data[0]['id'].'/'.$this->make_slug($data[0]['title']['th'])    );

        return view('pages.employeeDetail', [    'seo'       => $seo,
                                                'banner'    => $banner,
                                                'data'      => $data[0], 
                                                // 'gallery'   => $gallery,
                                                'services'  => $services, 
